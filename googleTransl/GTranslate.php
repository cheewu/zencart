...
function translate($string, $key)
 ...
 try{		
		...
		mysql_query("delete from products_description where language_id=2 and products_id =11004");
		//die;
foreach($arrKey as $keys =>$key){
		$rh=mysql_query("select products_id, products_name, products_description from products_description where language_id=1 and products_id =11004 order by products_id desc");
		...
		{	$products_description = '';
			...
			try{
				...
				*/
				$products_name=translate($rows['products_name'], $key);
