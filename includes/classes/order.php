...
*/
    $order_query = "select customers_id, customers_name, customers_company,
                         ...
                         payment_method, payment_module_code, shipping_method, shipping_module_code,
                         coupon_code, cc_type, cc_owner, cc_number, cc_expires, currency, currency_value,
                         date_purchased, orders_status, last_modified, order_total, order_tax, ip_address,billing_telephone,delivery_telephone
                        from " . TABLE_ORDERS . "
...
*/
    ...
    $this->delivery = array('name' => $order->fields['delivery_name'],
							...
							'tel' => $order->fields['billing_telephone'],
    ...
    $this->billing = array('name' => $order->fields['billing_name'],
                           ...
                           'country' => $order->fields['billing_country'],
							'tel' => $order->fields['delivery_telephone'],
...
*/
      ...
      if ($attributes->RecordCount()) {
        while (!$attributes->EOF) {
          $this->products[$index]['attributes'][$subindex] = array('option' => $attributes->fields['products_options'],
                                                                   ...
                                                                   //------- 这以下两个是 自己添加的,目的是将订单中的产品加入到购物车里
                                                                   'option_id' => $attributes->fields['products_options_id'],

                                                                   'values_id' => $attributes->fields['products_options_values_id']);
          $subindex++;
          $attributes->MoveNext();
        }
      }
  ...
  function cart() {
    ...
    $customer_address_query = "select c.customers_firstname, c.customers_lastname, c.customers_telephone,
                                   ...
                                   limit 1";    //修改这里防止default_address_id对不上号时 出现查不到邮箱的情况,需要的地址可以在shipping和billing里面找
    ...
    $this->info = array('order_status' => DEFAULT_ORDERS_STATUS_ID,
                        ...
                        'payment_method' => $selection[0]['id'],/*2010-9-17*/
                        'payment_module_code' => $selection[0]['title'],/*2010-9-17$GLOBALS[$class]->code*/
    ...
    //                          'cc_cvv' => (isset($GLOBALS['cc_cvv']) ? $GLOBALS['cc_cvv'] : ''),
                        ...
                        'total' => 0,
                        'tax_groups' => array(),
                        'comments' => (isset($_SESSION['comments']) ? $_SESSION['comments'] : $_POST['comments']),
...
*/
    ...
    $this->delivery = array('firstname' => $shipping_address->fields['entry_firstname'],
                            ...
                            'tel' => $shipping_address->fields['entry_telephone']);
    $this->billing = array('firstname' => $billing_address->fields['entry_firstname'],
                           ...
                           'tel' => $billing_address->fields['entry_telephone']);
    $index = 0;
...
*/
  ...
  function create($zf_ot_modules, $zf_mode = 2) {
    ...
    $sql_data_array = array("orders_id" => $ordernum,
							...
							'delivery_telephone' => $this->delivery['tel'],
							...
							'billing_telephone' => $this->billing['tel'],
                            'payment_method' => (($this->info['payment_module_code'] == '' and $this->info['payment_method'] == '') ? PAYMENT_METHOD_GV : $this->info['payment_method']),/*2010-9-17*/
                            'payment_module_code' =>(($this->info['payment_module_code'] == '' and $this->info['payment_method'] == '') ? PAYMENT_MODULE_GV : $this->info['payment_module_code']),/*2010-9-17*/
	...
	$insert_id = $ordernum;
	$_SESSION['create_order_no']=$ordernum;
    ...
    $crm_carrier = '';
    /*这里也很慢
    foreach($this->products as $crm_pro){
        $crm_i ++;
       ...
       // $crm_pro_string .= $crm_pro['qty'] . 'x' . $crm_pro['name'] . '(' . $crm_pro['model'] . ')'
        //                    . $currencies->display_price($crm_pro['final_price'], $crm_pro['tax']) . "\n";
    }
    */

    if($this->info['shipping_module_code'] == 'item_item'){
        $crm_carrier = "UPS快递";
    } else if($this->info['shipping_module_code'] == 'freeoptions_freeoptions'){
        $crm_carrier = "EMS";
    }

//    echo 'Products','LV_' . $crm_pro['id'];
//    echo '<br />' . $_SESSION['customer_id'] . '<br />';
//die($crm_pro_string);
    $crm_array = array();
	/*
    $crm_array = array('websiteid'=> STORE_NAME . '_' . $ordernum,       //网站标识+'_'+id,如ugg_34
                       'Subject'=>  STORE_NAME . '_' . $ordernum,    //* 商机名
                       'Account Name'=> $this->customer['email_address'],       //* 客户名
                       'Carrier'=> $crm_carrier,
                       'order date'=> date("Y-m-d"),
                       'Status'=>"创建" ,
                       'CONTACTID'=>crm_curl_getid('Contacts',STORE_NAME . '_' . $_SESSION['customer_id'],'websiteid'),
                       'SMOWNERID'=>SERVER_NO,
                       '支付'=>$this->info['payment_method'],
                       'Description'=> addslashes(str_replace('&','',$this->info['comments'])),
                       'Product Details'=>$crm_pro_string,

                       'Sub Total' => $this->info['subtotal'],
                       'Discount' => '0',
                       'Tax' => $this->info['tax'],
                       'Adjustment' => $this->info['shipping_cost'],
                       'Grand Total' => $this->info['total'],

                       'Shipping Name' => addslashes($this->delivery['firstname'] . ' ' . $this->delivery['lastname']),
                       'Shipping Street' => addslashes($this->delivery['street_address']),
                       'Shipping City' => addslashes($this->delivery['city']),
                       'Shipping Code' => addslashes($this->delivery['postcode']),
                       'Shipping State' => addslashes($this->delivery['state']),
                       'Shipping Country' => addslashes($this->delivery['country']['title']),
                       'Shipping Tel' => addslashes($this->delivery['tel']),
                       'Billing Name' => addslashes($this->billing['firstname'] . ' ' . $this->billing['lastname']),
                       'Billing Street' => addslashes($this->billing['street_address']),
                       'Billing City' => addslashes($this->billing['city']),
                       'Billing Code' => addslashes($this->billing['postcode']),
                       'Billing State' => addslashes($this->billing['state']),
                       'Billing Country' => addslashes($this->billing['country']['title']),
                       'Billing Tel' => addslashes($this->billing['tel'])
                );//die(crm_curl_getid('Contacts',STORE_NAME . '_' . $_SESSION['customer_id'],'websiteid'));
	*/

   $crm_array = array('websiteid'=> STORE_NAME . '_' . $ordernum,       //网站标识+'_'+id,如ugg_34
                       'Subject'=>  STORE_NAME . '_' . $ordernum,    //* 商机名
                       'Account Nam'=> $this->customer['email_address'],       //* 客户名
                       'Carrier'=> $crm_carrier,
                       'order date'=> date("y-m-d"),
                       'Status'=>"创建" ,
                       //'CONTACTID'=>crm_curl_getid('Contacts',STORE_NAME . '_' . $_SESSION['customer_id'],'websiteid'),转去其他地方生成
                       'CONTACTID' => '',
                       'SMOWNERID'=>SERVER_NO,
                       '支付'=>$this->info['payment_method'],
                       'Description'=> addslashes(str_replace('&','',$this->info['comments'])),
                       ...
                       'Billing Country' => addslashes($this->billing['country']['title']),
                       'Billing Tel' => addslashes($this->billing['tel'])
                );//print_r($crm_array);die('qqqqq' . crm_curl_getid('Contacts',STORE_NAME . '_' . $_SESSION['customer_id'],'websiteid') . 'zzzzz');
    $json_array = array("action" => "SalesOrders",
                        "data" => $crm_array,
                        "method" => 'insert',
    ...
    //crm_curl_post('SalesOrders',$crm_array);//die($this->customer['email_address']);
	//zen_db_perform("crm_info", $crm_array);
    } catch (Exception $e){
    }
...
/* END: ADD MY CUSTOM DETAILS */
  ...
  function send_order_email($zf_insert_id, $zf_mode) {
    ...
    $html_msg['INTRO_URL_VALUE']       = zen_href_link(FILENAME_ACCOUNT_HISTORY_INFO, 'order_id=' . $zf_insert_id, 'SSL', false);
        ...
        //------------------- add 2010 5 4 ------------------------------//
    if(defined('EMAIL_PAYMENT_WEBSRC_LINK')){
        $html_msg['EMAIL_PAYMENT_WEBSRC_LINK'] = "\n\n" . EMAIL_PAYMENT_WEBSRC_LINK . "\n\n";
    }else{
        $html_msg['EMAIL_PAYMENT_WEBSRC_LINK'] = '';
    }
    $email_order .= $html_msg['EMAIL_PAYMENT_WEBSRC_LINK'];
    //---------------------------------------------------------------//
    ...
    if ($this->info['comments']) {
      ...
      $email_order .= strip_tags(zen_db_output($this->info['comments'])) . "\n\n";      //这里进行修改,目的是单过滤注释,后面有个大的过滤我not allow 了a和img
      $html_msg['ORDER_COMMENTS'] = nl2br(strip_tags(zen_db_output($this->info['comments'])));
    } else {
