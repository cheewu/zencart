 ...
 */
  ...
  function resizing_allowed() {
    ...
    if ($transparent && ($file_ext == ".gif")) {
      ...
      $bg = trim(preg_replace('/transparent/', '', $bg));
  ...
  function resize_imageGD($file_ext, $dest_name, $bg, $quality = 85) {
    ...
    if ($this->sizetype == 'small' ||$this->sizetype == 'generic') {
      if ($ihConf[$this->sizetype]['zoom']) {
        ...
        $zoom_sizetype = ($this->sizetype=='small' ||$this->sizetype=='generic')?'medium':'large';
        ...
        $products_image_zoom = str_replace("/org/","/large/",$products_image_zoom);
        $zoomwidth = $zoomwidth?$zoomwidth:300;
        $zoomheight = $zoomheight?$zoomheight:300;
