...
//						case ($page == FILENAME_PRODUCT_INFO && !$this->is_attribute_string($params)):
		...
		if($page == FILENAME_DEFAULT)
		{
			if($container['style_name']&&$container['style'])
			{

				//$url.=str_replace(".html","-".$container['page'].".html",$url);
				$url.="/".$container['style_name']."_s".$container['style'];

				unset($container['style_name']);
				unset($container['style']);

			}
			if($container['page'])
			{
				$url.="/page-".$container['page'];
 ...
 */	
	function strip($string){
		...
		$pattern = $this->attributes['SEO_REMOVE_ALL_SPEC_CHARS'] == 'true'
						?	"/([^[:alnum:]])+/"
						:	"/([[:punct:]])+/";
		//$anchor = preg_replace($pattern, '', GBcase($string,"lower"));  // modified by zen-cart.cn
