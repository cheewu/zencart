...
// Text Mode
    ...
    $address_out = ADDRESS_NAME . "$firstname $lastname$cr"
                    . ADDRESS_STREET . "$streets$cr" . ADDRESS_CITY . "$city$cr" . ADDRESS_POSTCODE . "$postcode$cr".ENTRY_TELEPHONE_NUMBER."$telephone$cr"
...
// TABLES: customers, address_book
  function zen_address_label($customers_id, $address_id = 1, $html = false, $boln = '', $eoln = "\n") {
    ...
    $address_query = "select entry_firstname as firstname, entry_lastname as lastname,
                             entry_company as company,entry_telephone as telephone,entry_street_address as street_address,
    ...
    $str = zen_draw_hidden_field('shipping_address_id', $address->fields['address_book_id']);   //---
    $format_id = zen_get_address_format_id($address->fields['country_id']);
    return zen_address_format($format_id, $address->fields, $html, $boln, $eoln) . $str;
  }
  function zen_billing_address_label($customers_id, $address_id = 1, $html = false, $boln = '', $eoln = "\n") {
    global $db;
    $address_query = "select entry_firstname as firstname, entry_lastname as lastname,entry_telephone as telephone,
                      ...
                      and address_type = 1";
                      //and address_book_id = '" . (int)$address_id . "'";      ޸billingַֻһ,Բȥµĵַ
    $address = $db->Execute($address_query);
    $str = zen_draw_hidden_field('billing_address_id', $address->fields['address_book_id']);    //---------
    $format_id = zen_get_address_format_id($address->fields['country_id']);
    return zen_address_format($format_id, $address->fields, $html, $boln, $eoln) . $str;
  }
////
// Return a customer greeting
  ...
  function zen_count_customer_address_book_entries($id = '', $check_session = true) {
    ...
    $addresses_query = "select count(*) as total
                        from " . TABLE_ADDRESS_BOOK . "
                        where customers_id = '" . (int)$id . "' and address_type = 0";
