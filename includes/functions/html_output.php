...
// The HTML href link wrapper function
  function zen_href_link($page = '', $parameters = '', $connection = 'NONSSL', $add_session_id = true, $search_engine_safe = true, $static = false, $use_dir_ws_catalog = true,$useseo=true) {
		...
		if((SEO_URLS_ONLY_IN!="") && !in_array($page,$sefu) ||$useseo==false) {
...
//auto replace with defined missing image
    ...
    if($width == SMALL_IMAGE_WIDTH && $height == SMALL_IMAGE_HEIGHT){
        $src = str_replace("/org/","/small/",$src);
    } else if($width == MEDIUM_IMAGE_WIDTH && $height == MEDIUM_IMAGE_HEIGHT){
        $src = str_replace("/org/","/large/",$src);
    } else if($width == 150 && $height == 150){
        $src = str_replace("/org/","/medium/",$src);
    }
...
function zen_define_image($src,$path,$alt='',$parameters='')
{
	//$src=str_replace("images/","images/".$path."/",$src);
    $src=str_replace("/org/","/" . $path . "/",$src);
	$image = '<img src="' . zen_output_string($src) . '" alt="' . zen_output_string($alt) . '"';
	if (zen_not_null($parameters)) $image .= ' ' . $parameters;
	$image .= ' />';
	return $image;
