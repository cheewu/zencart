  ...
  if ($featured_products_split->number_of_rows > 0) {
    ...
    while (!$featured_products->EOF) {
      ...
      if (PRODUCT_FEATURED_LIST_NAME != '0') {
        $display_products_name = '<a href="' . zen_href_link(zen_get_info_page($featured_products->fields['products_id']), 'cPath=' . zen_get_generated_category_path_rev($featured_products->fields['master_categories_id']) . '&products_id=' . $featured_products->fields['products_id']) . '">' . $featured_products->fields['products_name'] . '</a>' . str_repeat('<br clear="all" />', substr(PRODUCT_FEATURED_LIST_NAME, 3, 1));
      } else {
...
?>
        ...
        echo '<span class="car_price">' . $display_products_price . '</span>';
        echo '</div>';
        ?>
        </ul></div></li>
              <?php /*
