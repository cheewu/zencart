	...
	switch($_GET["feed"])
	{
		case "news":
			...
			while (!$date_selector->EOF)
			{
				...
				while (!$article->EOF)
				{
					...
					$article->fields['author_name'] . " <" . $article->fields['author_email'] . ">",false,false,$xtags);
					$article->MoveNext();
				}
		...
		case "categories":
			...
			if ($check_categories->RecordCount() > 0)
			{
				if(isset($_GET['cPath']))
				{
					$categories = $db->Execute("SELECT c.categories_id, c.parent_id, GREATEST(c.date_added, IFNULL(c.last_modified, 0)) AS categories_date, c.categories_image, cd.categories_name, cd.categories_description FROM " . TABLE_CATEGORIES . " c LEFT JOIN " . TABLE_CATEGORIES_DESCRIPTION . " cd on c.categories_id = cd.categories_id WHERE c.parent_id = " . (int)$id_parent . " AND cd.language_id = " . (int)$_SESSION['languages_id'] . " AND c.categories_status= '1' ORDER BY c.sort_order, cd.categories_name",'', false, 150);
					if(!$categories->EOF)
					{
		...
		case "new_products_random":
			...
			$new_products_query = "select DISTINCT p.products_id, pd.products_name, pd.products_description, p.products_image, GREATEST(p.products_date_added, IFNULL(p.products_last_modified, 0)) AS products_date, p.master_categories_id, p.products_price_sorter AS price, p.products_tax_class_id, p.products_quantity, p.products_model, p.products_weight, p.manufacturers_id
			from (" . TABLE_PRODUCTS . " p
			...
			and pd.language_id = '" . (int)$_SESSION['languages_id'] . "'";
			zen_rss_products($new_products_query, $random);
			break;
		case "products":
		default:
			$where_cat = $from_cat = "";
			if (isset($current_category_id) && ($current_category_id > 0))
			{
				...
				{
					$from_cat = ", " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c, " . TABLE_CATEGORIES . " c ";
					$where_cat = "AND p.products_id = p2c.products_id AND c.categories_id = " . (int)$current_category_id . " AND p2c.categories_id = " . (int)$current_category_id . " ";
				}
				else
				{
					$where_cat = "AND p.master_categories_id = " . (int)$current_category_id . " ";
...
function zen_rss_products($sql_products, $random)
{
	...
	$sql_maxdate = "SELECT GREATEST(MAX(products_date_added), MAX(IFNULL(products_last_modified, 0))) as max_date FROM " . TABLE_PRODUCTS . " WHERE products_status = 1";
	...
	while(!$products->EOF)
	{
		...
		if(RSS_PRODUCTS_RATING == 'true' && zen_not_null($products->fields['reviews_rating'])) $xtags['g:rating'] = $products->fields['reviews_rating'];
		...
		{
			$image_url = zen_image(DIR_WS_IMAGES . $products->fields['products_image'], $products->fields['products_name'], MEDIUM_IMAGE_WIDTH, MEDIUM_IMAGE_HEIGHT, 'style="float: left; margin: 0px 8px 8px 0px;"');
