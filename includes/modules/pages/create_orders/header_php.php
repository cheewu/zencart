...
require_once(DIR_WS_CLASSES . 'payment.php');
  ...
  if($_POST['act']=='insert_payment')
  {
	  //print_r($_POST);
	  $order_no=zen_db_output($_POST['orders_no']);
	  $payment_order=$db->Execute("select * from ".TABLE_ORDERS." where orders_id='".$order_no."'");
	  
	  if($payment_order->RecordCount()>0)
	  {
			$payment_modules = new payment($_POST['paymentd']);
			$paymentd=$payment_modules->selected_module;
			$payment_module_code=$$paymentd->title;
			$db->Execute("update ".TABLE_ORDERS." set payment_method='".$_POST['paymentd']."',payment_module_code='".$payment_module_code."' where orders_id='".$order_no."'");
			echo 'ok';
	  }
	  else
	  {
...
//-------------------------------------------- 验证eof -----------------------------------------------------//
	...
	*/
	...
	if(isset($_POST['source'])&&$_POST['source'] == 'pay_orders'){
//------------------------------ shipping --------------------------------------//
