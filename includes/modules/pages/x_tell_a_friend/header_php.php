...
if (isset($_GET['action']) && ($_GET['action'] == 'process'))
{
  ...
  if ($error == false) {
    ...
    //$email_body .= sprintf(EMAIL_TEXT_LINK, zen_href_link(zen_get_info_page($_GET['products_id']), 'uid='.$_SESSION['customer_id'].'&products_id=' . $_GET['products_id']), '', false) . "\n\n" .

    $email_body .= sprintf(EMAIL_TEXT_LINK, zen_href_link(FILENAME_CREATE_ACCOUNT,'ref='.$_SESSION['customer_id']), '', false) . "\n\n" . 
	sprintf(EMAIL_TEXT_SIGNATURE, STORE_NAME . "\n" . HTTP_SERVER . DIR_WS_CATALOG . "\n");

    $html_msg['EMAIL_TEXT_HEADER'] = EMAIL_TEXT_HEADER;
    $html_msg['EMAIL_PRODUCT_LINK'] = sprintf(str_replace('\n\n','<br />',EMAIL_TEXT_LINK), '<a href="'.zen_href_link(FILENAME_CREATE_ACCOUNT,'ref='.$_SESSION['customer_id']).'">'.STORE_NAME.'</a>' , '', false);
    ...
    zen_redirect(zen_href_link(FILENAME_X_TELL_A_FRIEND));
  }
}
