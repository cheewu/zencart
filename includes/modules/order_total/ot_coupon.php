  ...
  function credit_selection() {
    ...
    $selection = array('id' => $this->code,
                       ...
                       'fields' => array(array('title' => ($discount_coupon->fields['coupon_code'] != '' ? MODULE_ORDER_TOTAL_COUPON_TEXT_CURRENT_CODE . '<a href="javascript:couponpopupWindow(\'' . zen_href_link(FILENAME_POPUP_COUPON_HELP, 'cID=' . $_SESSION['cc_id']) . '\')">' . $discount_coupon->fields['coupon_code'] . '</a><br />' : '') . MODULE_ORDER_TOTAL_COUPON_TEXT_ENTER_CODE,
                                               'field' => zen_draw_input_field('dc_redeem_code', $discount_coupon->fields['coupon_code'], 'id="disc-' . $this->code . '" onkeyup="submitFunction(0,0)"'),
                                               'tag' => 'disc-'.$this->code
                       )));
    return $selection;
  }
  ...
  function collect_posts()
{
    ...
    global $discount_coupon;
    global $other_href;
    $coupon_href = $other_href?$other_href:FILENAME_CHECKOUT_PAYMENT;   //---------- 增加可以修改打折码出错后的跳转页
    $_SESSION['dc_redeem_code'] = $_POST['dc_redeem_code']; //---------------- 把打折计入session,因为到checkout里的时候没有用到post

    // remove discount coupon by request
    if (isset($_POST['dc_redeem_code']) && strtoupper($_POST['dc_redeem_code']) == 'REMOVE') {
      unset($_POST['dc_redeem_code']);
      unset($_SESSION['cc_id']);
      $messageStack->add_session('checkout_payment', TEXT_REMOVE_REDEEM_COUPON, 'caution');//Discount Coupon Removed by Request!
    }
    ...
    if ($_SESSION['cc_id'] > 0) {
      ...
      if (!$foundvalid) {
        $messageStack->add_session('checkout_payment', TEXT_REMOVE_REDEEM_COUPON_ZONE, 'caution');//The coupon code you entered is not valid for the address you have selected.
        $this->clear_posts();
        if (!$foundvalid) zen_redirect(zen_href_link($coupon_href, '', 'SSL',true, false));
      }
    }
    ...
    if ((isset($_POST['dc_redeem_code']) && $_POST['dc_redeem_code'] != '') || (isset($discount_coupon->fields['coupon_code']) && $discount_coupon->fields['coupon_code'] != '')) {
      ...
      if ($coupon_result->fields['coupon_type'] != 'G') {
        ...
        if ($coupon_result->RecordCount() < 1 ) {
          ...
          $this->clear_posts();
          zen_redirect(zen_href_link($coupon_href, '', 'SSL',true, false));
        }


        $order_total = $this->get_order_total();
        if ($order_total['totalFull'] < $coupon_result->fields['coupon_minimum_order']) {
          $messageStack->add_session('redemptions', sprintf(TEXT_INVALID_REDEEM_COUPON_MINIMUM, $currencies->format($coupon_result->fields['coupon_minimum_order'])),'caution');//You must spend at least %s to redeem this coupon
        ...
        if (!$foundvalid) zen_redirect(zen_href_link($coupon_href, 'credit_class_error_code=' . $this->code . '&credit_class_error=' . urlencode(TEXT_INVALID_COUPON_PRODUCT . ' ' . $dc_check), 'SSL',true, false));
        ...
        if ($date_query->RecordCount() < 1 ) {
          $messageStack->add_session('redemptions', TEXT_INVALID_STARTDATE_COUPON,'caution');//This coupon is not available yet
        ...
        if ($date_query->RecordCount() < 1 ) {
          $messageStack->add_session('redemptions', TEXT_INVALID_FINISHDATE_COUPON,'caution');//This coupon has expired
        ...
        if ($coupon_count->RecordCount() >= $coupon_result->fields['uses_per_coupon'] && $coupon_result->fields['uses_per_coupon'] > 0) {
          $messageStack->add_session('redemptions', TEXT_INVALID_USES_COUPON . $coupon_result->fields['uses_per_coupon'] . TIMES ,'caution');//This coupon could only be used 
        ...
        if ($coupon_count_customer->RecordCount() >= $coupon_result->fields['uses_per_user'] && $coupon_result->fields['uses_per_user'] > 0) {
          $messageStack->add_session('redemptions', sprintf(TEXT_INVALID_USES_USER_COUPON, $dc_check) . $coupon_result->fields['uses_per_user'] . ($coupon_result->fields['uses_per_user'] == 1 ? TIME : TIMES) ,'caution');//You have used coupon code: %s the maximum number of times allowed per customer. 
      ...
      return $coupon_result->fields['coupon_amount'];
    }
  }
  /**
   * Enter description here...
