...
switch ($_GET['main_page']) {
  ...
  define('META_TAG_TITLE', HOME_PAGE_TITLE);
  define('META_TAG_DESCRIPTION', (HOME_PAGE_META_DESCRIPTION != '') ? HOME_PAGE_META_DESCRIPTION : TITLE . PRIMARY_SECTION . (defined('NAVBAR_TITLE') ? NAVBAR_TITLE : '' ) . SECONDARY_SECTION . KEYWORDS);
  define('META_TAG_KEYWORDS', (HOME_PAGE_META_KEYWORDS != '') ? HOME_PAGE_META_KEYWORDS : KEYWORDS . METATAGS_DIVIDER . (defined('NAVBAR_TITLE') ? NAVBAR_TITLE : '' ) );
  break;
  ...
  $meta=array("1"=>'Discount','2'=>'Cheap','3'=>'Retail','4'=>'Replica','5'=>'Quality','6'=>'On Sale','7'=>'New','8'=>'Buy');
  ...
  if (!$category_metatags->EOF)
	{
    define('META_TAG_TITLE',str_replace('"','',$meta_des.$category_metatags->fields['metatags_title']));
    ...
    if ($category_depth == 'nested') {//have sub category
      ...
      if ($category_metatags->EOF) {
        ...
        define('META_TAG_TITLE', "Louis Vuitton ".zen_clean_html(str_replace("-"," ",$_GET['style_name']))." ".str_replace('"','', zen_clean_html($category_metatags->fields['categories_name']) . PRIMARY_SECTION . TITLE . TAGLINE));
        define('META_TAG_DESCRIPTION', str_replace('"','',/*TITLE . PRIMARY_SECTION . */"Louis Vuitton ".zen_clean_html(str_replace("-"," ",$_GET['style_name']))." ".zen_clean_html($category_metatags->fields['categories_name']) . SECONDARY_SECTION . KEYWORDS));
        define('META_TAG_KEYWORDS', str_replace('"','',/*KEYWORDS . METATAGS_DIVIDER . */"Louis Vuitton ".zen_clean_html(str_replace("-"," ",$_GET['style_name']))." ".zen_clean_html($category_metatags->fields['categories_name'])));
      } // EOF
    }
	elseif ($category_depth == 'products')
	{
      if (isset($_GET['manufacturers_id']) || ((isset($_GET['filter_id']) && $_GET['filter_id'] > 0) && isset($_GET['cPath'])) ) {
		...
		{
          define('META_TAG_TITLE', str_replace('"','', "Louis Vuitton ".zen_clean_html(str_replace("-"," ",$_GET['style_name']))." ".zen_clean_html($category_metatags->fields['categories_name']) . PRIMARY_SECTION . TITLE . TAGLINE));
          define('META_TAG_DESCRIPTION', str_replace('"','',/*TITLE . PRIMARY_SECTION .*/"Louis Vuitton ". zen_clean_html(str_replace("-"," ",$_GET['style_name']))." ".zen_clean_html($category_metatags->fields['categories_name']) . SECONDARY_SECTION . KEYWORDS));
          define('META_TAG_KEYWORDS', str_replace('"','',/*KEYWORDS . METATAGS_DIVIDER .*/"Louis Vuitton ".zen_clean_html(str_replace("-"," ",$_GET['style_name']))." ". zen_clean_html($category_metatags->fields['categories_name'])));
        } // EOF
      }
...
/////////////////////////////////////////
  case 'search':
  define('META_TAG_TITLE', str_replace("-"," ",$_GET['keyword']) . (defined('NAVBAR_TITLE') ? NAVBAR_TITLE . PRIMARY_SECTION : '') . TITLE . TAGLINE);
  define('META_TAG_DESCRIPTION',str_replace("-"," ",$_GET['keyword']) .  TITLE . PRIMARY_SECTION . (defined('NAVBAR_TITLE') ? NAVBAR_TITLE : '' ) . SECONDARY_SECTION . KEYWORDS);
  define('META_TAG_KEYWORDS',str_replace("-"," ",$_GET['keyword']) .  KEYWORDS . METATAGS_DIVIDER . (defined('NAVBAR_TITLE') ? NAVBAR_TITLE : '' ) );
  break;


  case 'tags':
      define('META_TAG_TITLE',META_TAG_TITLE_TAGS);
      define('META_TAG_DESCRIPTION',META_TAG_DESCRIPTION_TAGS);
      define('META_TAG_KEYWORDS',META_TAG_KEYWORDS_TAGS);
  break;
 case 'products_new':
  	  define('META_TAG_TITLE', 'nuovi arrivi - Borse Firmate online Outlet');
  	define('META_TAG_DESCRIPTION', 'nuovi arrivi Borsa Pelle Milano');
  	define('META_TAG_KEYWORDS', KEYWORDS . METATAGS_DIVIDER . (defined('NAVBAR_TITLE') ? NAVBAR_TITLE : '' ) );
  	break;
  case 'products_new':
  	  define('META_TAG_TITLE', 'prodotti in vetrina - Borse Firmate online Outlet');
  	define('META_TAG_DESCRIPTION', 'prodotti in vetrina Borse Firmate online Outlet');
  	define('META_TAG_KEYWORDS', KEYWORDS . METATAGS_DIVIDER . (defined('NAVBAR_TITLE') ? NAVBAR_TITLE : '' ) );
  	break;
  case 'conditions':
  	  define('META_TAG_TITLE', "condizioni dell'utilizzo");
  	define('META_TAG_DESCRIPTION', TITLE . PRIMARY_SECTION . (defined('NAVBAR_TITLE') ? NAVBAR_TITLE : '' ) . SECONDARY_SECTION . KEYWORDS);
  	define('META_TAG_KEYWORDS', KEYWORDS . METATAGS_DIVIDER . (defined('NAVBAR_TITLE') ? NAVBAR_TITLE : '' ) );
  	break;
  case 'conditions':
  	die;
  	define('META_TAG_TITLE', "spedizione");
  	define('META_TAG_DESCRIPTION', TITLE . PRIMARY_SECTION . (defined('NAVBAR_TITLE') ? NAVBAR_TITLE : '' ) . SECONDARY_SECTION . KEYWORDS);
  	define('META_TAG_KEYWORDS', KEYWORDS . METATAGS_DIVIDER . (defined('NAVBAR_TITLE') ? NAVBAR_TITLE : '' ) );
  	break;
    case 'privacy':
  	  define('META_TAG_TITLE', "privata");
  	define('META_TAG_DESCRIPTION', TITLE . PRIMARY_SECTION . (defined('NAVBAR_TITLE') ? NAVBAR_TITLE : '' ) . SECONDARY_SECTION . KEYWORDS);
  	define('META_TAG_KEYWORDS', KEYWORDS . METATAGS_DIVIDER . (defined('NAVBAR_TITLE') ? NAVBAR_TITLE : '' ) );
  	break;
	case 'shippinginfo':
  	  define('META_TAG_TITLE', "Spedizioni e Resi");
  	define('META_TAG_DESCRIPTION', TITLE . PRIMARY_SECTION . (defined('NAVBAR_TITLE') ? NAVBAR_TITLE : '' ) . SECONDARY_SECTION . KEYWORDS);
  	define('META_TAG_KEYWORDS', KEYWORDS . METATAGS_DIVIDER . (defined('NAVBAR_TITLE') ? NAVBAR_TITLE : '' ) );
  	break;
   case 'site_map':
  	  define('META_TAG_TITLE', "Mappa del Sito");
  	define('META_TAG_DESCRIPTION', TITLE . PRIMARY_SECTION . (defined('NAVBAR_TITLE') ? NAVBAR_TITLE : '' ) . SECONDARY_SECTION . KEYWORDS);
  	define('META_TAG_KEYWORDS', KEYWORDS . METATAGS_DIVIDER . (defined('NAVBAR_TITLE') ? NAVBAR_TITLE : '' ) );
  	break;
     case 'contact_us':
  	  define('META_TAG_TITLE', "Contatti");
  	define('META_TAG_DESCRIPTION', TITLE . PRIMARY_SECTION . (defined('NAVBAR_TITLE') ? NAVBAR_TITLE : '' ) . SECONDARY_SECTION . KEYWORDS);
  	define('META_TAG_KEYWORDS', KEYWORDS . METATAGS_DIVIDER . (defined('NAVBAR_TITLE') ? NAVBAR_TITLE : '' ) );
  	break;
////////////////////////////////////////
