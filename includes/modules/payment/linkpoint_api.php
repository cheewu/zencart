...
//  Store Transaction history in Database
  ...
  function install() {
    global $db;
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('启用Linkpoint支付模块', 'MODULE_PAYMENT_LINKPOINT_API_STATUS', 'True', '是否启用Linkpoint信用卡支付?', '6', 121, 'zen_cfg_select_option(array(\'True\', \'False\'), ', now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Linkpoint/YourPay商户编号', 'MODULE_PAYMENT_LINKPOINT_API_LOGIN', 'EnterYourStoreNumber', '请输入您的 Linkpoint/YourPay 商户编号。<br />与您帐号的PEM数字证书文件名中的数字一样。', '6', 121, now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('LinkPoint交易模式响应', 'MODULE_PAYMENT_LINKPOINT_API_TRANSACTION_MODE_RESPONSE', 'LIVE: Production', '<strong>正式:</strong> 正式运营的网店使用。<br />如果要测试模块，选择这些选项:<br /><strong>Successful:</strong> 用于测试一个成功的交易<br /><strong>Decline:</strong> 用于测试一个失败的交易', '6', 121, 'zen_cfg_select_option(array(\'LIVE: Production\', \'TESTING: Successful\', \'TESTING: Decline\'), ', now())");
    ...
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('启用数据库存储', 'MODULE_PAYMENT_LINKPOINT_API_STORE_DATA', 'True', '保存网关通信数据到数据库中。在 管理页面->客户管理->Linkpoint CC记录中查看。', '6', 121, 'zen_cfg_select_option(array(\'True\', \'False\'), ', now())");
