...
switch ($_GET['action']) {
  case 'update' :
    ...
    if($_POST['quick_updates_new']['products_upc']){;
    	foreach($_POST['quick_updates_new']['products_upc'] as $products_id => $new_value) {
    		if (trim($_POST['quick_updates_new']['products_upc'][$products_id]) != trim($_POST['quick_updates_old']['products_upc'][$products_id])) {
          		$quick_updates_count['products_upc'][$products_id] = $products_id;
          		$db->Execute("UPDATE " . TABLE_PRODUCTS . " SET products_upc='" . zen_db_input($new_value) . "', products_last_modified=NOW() WHERE products_id=" . (int)$products_id);
       	 	}
      	}
    }
   	
    if($_POST['quick_updates_new']['products_isbn']){;
    	foreach($_POST['quick_updates_new']['products_isbn'] as $products_id => $new_value) {
    		if (trim($_POST['quick_updates_new']['products_isbn'][$products_id]) != trim($_POST['quick_updates_old']['products_isbn'][$products_id])) {
          		$quick_updates_count['products_isbn'][$products_id] = $products_id;
          		$db->Execute("UPDATE " . TABLE_PRODUCTS . " SET products_isbn='" . zen_db_input($new_value) . "', products_last_modified=NOW() WHERE products_id=" . (int)$products_id);
       	 	}
      	}
    }
    
    //add UPC and ISBN BELOW
...
//// control string sort page
  ...
  $products_query_raw = "select p.products_id, p.products_type, p.products_image,
                                " . $extra_query . "
                                p.products_model, pd.products_name, p.products_status, p.products_upc, p.products_isbn,
...
?>
      ...
      <table class="quTop">
        <tr>
          ...
          <td class="smallText" align="center" valign="top"><?php echo zen_draw_form('categorie', FILENAME_QUICK_UPDATES, zen_get_all_get_params(array('cPath')), 'get') . DISPLAY_CATEGORIES . '&nbsp;&nbsp;' . zen_draw_pull_down_menu('cPath', zen_get_category_tree(), $current_category_id, 'onChange="this.form.submit();"') . '</form>'; ?></td>
          <td>
            <?php
...
if(QUICKUPDATES_MODIFY_UPC == 'true')
  echo zen_quickupdates_table_head('p.products_upc', TABLE_HEADING_UPC);
if(QUICKUPDATES_MODIFY_ISBN == 'true')
  echo zen_quickupdates_table_head('p.products_isbn', TABLE_HEADING_ISBN);
  ...
  if($prev_products_id != $products->fields['products_id']){
    ...
    if(QUICKUPDATES_MODIFY_MODEL == 'true') {
      ...
      echo '</td>' . "\n";
   	...
   	if(QUICKUPDATES_MODIFY_UPC == 'true') {
		echo '<td class="smallText productsUPC">';
	    echo zen_draw_input_field('quick_updates_new[products_upc][' . $products->fields['products_id'] . ']', stripslashes($products->fields['products_upc']), 'size="12"') . zen_draw_hidden_field('quick_updates_old[products_upc][' . $products->fields['products_id'] . ']', stripslashes($products->fields['products_upc']));  
	    // added for external links paulm
	    if (defined('QUICKUPDATES_MODIFY_UPC_INFO')){
	    	echo sprintf(QUICKUPDATES_MODIFY_UPC_INFO, $products->fields['products_id'], stripslashes($products->fields['products_upc']), zen_image(DIR_WS_IMAGES . 'icon_info.gif', stripslashes($products->fields['products_upc'])));
	    }
	   	echo '</td>' . "\n";
    }

    if(QUICKUPDATES_MODIFY_ISBN == 'true') {
		echo '<td class="smallText productsISBN">';
	    echo zen_draw_input_field('quick_updates_new[products_isbn][' . $products->fields['products_id'] . ']', stripslashes($products->fields['products_isbn']), 'size="12"') . zen_draw_hidden_field('quick_updates_old[products_isbn][' . $products->fields['products_id'] . ']', stripslashes($products->fields['products_isbn']));  
	    // added for external links paulm
	    if (defined('QUICKUPDATES_MODIFY_ISBN_INFO')){
	    	echo sprintf(QUICKUPDATES_MODIFY_ISBN_INFO, $products->fields['products_id'], stripslashes($products->fields['products_isbn']), zen_image(DIR_WS_IMAGES . 'icon_info.gif', stripslashes($products->fields['products_isbn'])));
	    }
	   	echo '</td>' . "\n";
    }
      ...
      */
      ...
      if($_SESSION['quick_updates']['categories_switch'] == 'master_cats'){
        ...
        if($invalidcat == true){
          ...
          echo zen_draw_pull_down_menu('quick_updates_new[categories_id][' . $products->fields['products_id'] . ']', zen_get_category_tree(), $products->fields['categories_id'], '');
        ...
        <tr>
          ...
          <td>
            ...
            //export_products[$products->fields['products_id']]['products_model']
             ...
             foreach((array)$export_products as $key => $value){
               if(!empty($value['products_upc'])) $text = ' (' . $value['products_upc'] . ')';
               $quick_copy_from_array[] = array(
                                            'id' => $key,
                                            'text' => 'id:' . $key . $text
                                            );
             ...
             foreach((array)$export_products as $key => $value){
               if(!empty($value['products_isbn'])) $text = ' (' . $value['products_isbn'] . ')';
               $quick_copy_from_array[] = array(
                                            'id' => $key,
                                            'text' => 'id:' . $key . $text
                                            );
