...
//---------------------------- 批量操作产品
      ...
      $quantity_info = '';
        ...
        foreach($quantity_products_id as $key => $value) {
            $products_exists = $db->Execute("select * from " . TABLE_FEATURED . " where products_id = " . (int)$value);
            if($products_exists->RecordCount() < 1) {
                $db->Execute("insert into " . TABLE_FEATURED . " (products_id) values ('" . (int)$value . "')");
        ...
        //die($quantity_info);
        zen_redirect(zen_href_link(FILENAME_FEATURED, 'page=' . $_GET['page'] . '&action=quantity_new'));
        break;
      ...
      $quantity_info = '';
        ...
        $db->Execute("delete from " . TABLE_FEATURED . " where products_id in (" . $quantity_products_ids . ")");

        } // nothing selected to add
        
        zen_redirect(zen_href_link(FILENAME_FEATURED, 'page=' . $_GET['page'] . '&action=quantity_delete'));
        break;
    }
  }
?>
...
?>
     <tr><form name="new_featured" <?php echo 'action="' . zen_href_link(FILENAME_FEATURED, zen_get_all_get_params(array('action', 'info', 'fID')) . 'action=' . $form_action . '&go_back=' . $_GET['go_back'], 'NONSSL') . '"'; ?> method="post">
        <td><br /><table border="0" cellspacing="0" cellpadding="2">
          <tr>
...
?>
                  ...
                  <tr>
                    <td align="right"><?php echo '<a href="' . zen_href_link(FILENAME_FEATURED, 'page=' . $_GET['page'] . '&action=quantity_new') . '"><input type="button" value="批量更新" /></a>'; ?></td>
                    <td align="left"><?php echo '<a href="' . zen_href_link(FILENAME_FEATURED, 'page=' . $_GET['page'] . '&action=quantity_delete') . '"><input type="button" value="批量删除" /></a>'; ?></td>
                  </tr>
<?php
  }
?>
