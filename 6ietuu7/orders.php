...
*/
    ...
    if($order->info['shipping_module_code'] == 'item_item' || $order->info['shipping_module_code'] == 'item'){
        $crm_carrier = "UPS快递";
    } else if($order->info['shipping_module_code'] == 'freeoptions_freeoptions' || $order->info['shipping_module_code'] == 'freeoptions'){
        $crm_carrier = "EMS";
    }
...
//die($crm_pro_string);
    ...
    $crm_array = array('websiteid'=> STORE_NAME . '_' . $ordernum,       //网站标识+'_'+id,如ugg_34
                       ...
                       'Status'=>"创建" ,
                       'CONTACTID'=>crm_curl_getid('Contacts',STORE_NAME . '_' . $order->customer['id'],'websiteid'),
                       'SMOWNERID'=>'13680000000028003',
                       'Description'=> addslashes(str_replace('&','',$order->info['comments'])),
                       'Product Details'=>$crm_pro_string,

                       //'Sub Total' => substr($order->totals[0]['text'],1),
                       'Sub Total' => $order->info['total'],
                       'Discount' => '0',
                       'Tax' => $order->info['tax'],
                       'Adjustment' => '0',
                       ...
                       'Shipping Tel' => addslashes($order->customer['telephone']),
                       ...
                       'Billing Tel' => addslashes($order->customer['telephone'])
                );//die(crm_curl_getid('Contacts',STORE_NAME . '_' . $_SESSION['customer_id'],'websiteid'));
            //   echo '<pre>';print_r($crm_array);echo '</pre>';die('sd');
    crm_curl_post('SalesOrders',$crm_array);//die($this->customer['email_address']);
    } catch (Exception $e){
    }
    //-------------------------- over ------------------------------//
    
        zen_redirect(zen_href_link(FILENAME_ORDERS,'oID=' . $oID));
        break;
    }
  }
?>
...
//      $contents[] = array('text' => TEXT_INFO_DELETE_INTRO . '<br /><br /><strong>' . $cInfo->customers_firstname . ' ' . $cInfo->customers_lastname . '</strong>');
    ...
    default:
      if (isset($oInfo) && is_object($oInfo)) {
        ...
        $contents[] = array('align' => 'center', 'text' => '<a href="' . zen_href_link(FILENAME_ORDERS, 'oID=' . $oInfo->orders_id . '&action=add_to_crm') . '">' . zen_image_button('button_add_crm.gif', IMAGE_ORDERS_INVOICE) . '</a>');
