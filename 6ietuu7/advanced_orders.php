 ...
 * @version $Id: orders.php 14753 2009-11-07 19:58:13Z drbyte $
 */
  ...
  if (isset($_GET['oID'])) {
   ...
   // $orders = $db->Execute("select orders_id from " . TABLE_ORDERS . " where orders_id = '" . (int)$oID . "'");
	...
	if($action=='all_del')
	{
		if($_POST['adv_orders_id'])
		{
			foreach($_POST['adv_orders_id'] as $value)
			{
				// demo active test
				if (zen_admin_demo()) {
				$_GET['action']= '';
					$messageStack->add_session(ERROR_ADMIN_DEMO, 'caution');
					zen_redirect(zen_href_link(FILENAME_ADVANCED_ORDERS, zen_get_all_get_params(array('oID', 'action')), 'NONSSL'));
				}
				$oID = zen_db_prepare_input($value);
				zen_remove_order($oID, $_POST['restock']);
				$messageStack->add_session("订单".$value."删除成功",'success');
			}

		}
		else
		{
			$messageStack->add_session("请选择要删除的订单", 'caution');
		}
		zen_redirect(zen_href_link(FILENAME_ADVANCED_ORDERS, zen_get_all_get_params(array('oID', 'action')), 'NONSSL'));
			exit;
	}
	if($action=='all_update')
	{
		if($_POST['adv_orders_id'])
		{
			foreach($_POST['adv_orders_id'] as $value)
			{
				...
				if (zen_admin_demo())
				{
					$_GET['action']= '';
					$messageStack->add_session(ERROR_ADMIN_DEMO, 'caution');
					zen_redirect(zen_href_link(FILENAME_ADVANCED_ORDERS, zen_get_all_get_params(array('action')) . '', 'NONSSL'));
				}
				$oID = zen_db_prepare_input($value);
				...
				$check_status = $db->Execute("select * from " . TABLE_ORDERS . " where orders_id = '" . $oID . "'");
				if(($check_status->fields['orders_status'] != $status) || zen_not_null($comments))
				{
					//$db->Execute("update " . TABLE_ORDERS . " set orders_status = '" . zen_db_input($status) . "', last_modified = now() where orders_id = '" . (int)$oID . "'");
					$db->Execute("update " . TABLE_ORDERS . " set orders_status = '" . zen_db_input($status) . "', last_modified = now() where orders_id = '" . $oID . "'");
					$notify_comments = '';
					...
					//$db->Execute("insert into " . TABLE_ORDERS_STATUS_HISTORY . " (orders_id, orders_status_id, date_added, customer_notified, comments) values ('" . (int)$oID . "', '" . zen_db_input($status) . "',now(), '" .zen_db_input($customer_notified) . "','" . zen_db_input($comments)  . "')");
					$db->Execute("insert into " . TABLE_ORDERS_STATUS_HISTORY . " (orders_id, orders_status_id, date_added, customer_notified, comments) values ('" . $oID . "', '" . zen_db_input($status) . "',now(), '" .zen_db_input($customer_notified) . "','" . zen_db_input($comments)  . "')");
					$order_updated = true;
				}
				...
				if($order_updated == true)
				{
					...
					$messageStack->add_session("订单 ".$value." 更新成功", 'success');
				}
				else
				{
					$messageStack->add_session("订单 ".$value." 没有变化", 'warning');
				}
    ...
    switch ($action) {
      ...
      // reset single download to on
        if ($_GET['download_reset_on'] > 0) {
          ...
          zen_redirect(zen_href_link(FILENAME_ADVANCED_ORDERS, zen_get_all_get_params(array('action')) . 'action=edit', 'NONSSL'));
        }
...
<!--弹出窗口插件-->
<script type="text/javascript" src="images/colorbox/jquery.colorbox.js"></script>
<link type="text/css" media="screen" rel="stylesheet" href="images/colorbox/colorbox.css" />
    ...
    <td width="100%" valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
      <tr>
        <td><!--<table border="0" width="100%" cellspacing="0" cellpadding="0">
         <tr><?php echo zen_draw_form('search', FILENAME_ADVANCED_ORDERS, '', 'get', '', true); ?>
...
// show reset search
  if ((isset($_GET['search']) && zen_not_null($_GET['search'])) or $_GET['cID'] !='') {
    echo '<a href="' . zen_href_link(FILENAME_ADVANCED_ORDERS, '', 'NONSSL') . '">' . zen_image_button('button_reset.gif', IMAGE_RESET) . '</a><br />';
  }
?>
<?php
...
?>
         ...
         <?php echo zen_draw_form('search_orders_products', FILENAME_ADVANCED_ORDERS, '', 'get', '', true); ?>
...
?>
          ...
          <tr>
            ...
            <td class="main"><?php echo $order->info['cc_number'] . (zen_not_null($order->info['cc_number']) && !strstr($order->info['cc_number'],'X') && !strstr($order->info['cc_number'],'********') ? '&nbsp;&nbsp;<a href="' . zen_href_link(FILENAME_ADVANCED_ORDERS, '&action=mask_cc&oID=' . $oID, 'NONSSL') . '" class="noprint">' . TEXT_MASK_CC_NUMBER . '</a>' : ''); ?><td>
          </tr>
          <tr>
            <td class="main"><?php echo ENTRY_CREDIT_CARD_CVV; ?></td>
            <td class="main"><?php echo $order->info['cc_cvv'] . (zen_not_null($order->info['cc_cvv']) && !strstr($order->info['cc_cvv'],TEXT_DELETE_CVV_REPLACEMENT) ? '&nbsp;&nbsp;<a href="' . zen_href_link(FILENAME_ADVANCED_ORDERS, '&action=delete_cvv&oID=' . $oID, 'NONSSL') . '" class="noprint">' . TEXT_DELETE_CVV_FROM_DATABASE . '</a>' : ''); ?><td>
          </tr>
          <tr>
...
<?php
    for ($i=0, $n=sizeof($order->products); $i<$n; $i++) {
      echo '          <tr class="dataTableRow">' . "\n" .
           ...
           '            <td class="dataTableContent" valign="top"><a href="'.HTTP_SERVER.'/index.php?main_page=product_info&products_id=' . $order->products[$i]['id'].'">' . $order->products[$i]['name']."</a>";
...
?>
      ...
      <tr><?php echo zen_draw_form('status', FILENAME_ADVANCED_ORDERS, zen_get_all_get_params(array('action')) . 'action=update_order', 'post', '', true); ?>
        ...
        <td width="100%"><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
            ...
            <td align="right"><table border="0" width="100%" cellspacing="0" cellpadding="0">
              <tr><?php echo zen_draw_form('orders', FILENAME_ADVANCED_ORDERS, '', 'get', '', true); ?>
              ...
              <tr><?php echo zen_draw_form('status', FILENAME_ADVANCED_ORDERS, '', 'get', '', true); ?>
...
?>
                <td width="28" align="center" class="dataTableHeadingContent">&nbsp;</td>
                <td width="125" align="center" class="dataTableHeadingContent"><?php echo TABLE_HEADING_ORDERS_ID; ?></td>
                <td class="dataTableHeadingContent" align="left" width="50"><?php echo TABLE_HEADING_PAYMENT_METHOD; ?></td>
                <td width="132" class="dataTableHeadingContent"><?php echo TABLE_HEADING_CUSTOMERS; ?></td>
                <td width="132" align="right" class="dataTableHeadingContent"><?php echo TABLE_HEADING_ORDER_TOTAL; ?></td>
                <td width="132" align="center" class="dataTableHeadingContent"><?php echo TABLE_HEADING_DATE_PURCHASED; ?></td>
                <td width="132" align="right" class="dataTableHeadingContent"><?php echo TABLE_HEADING_STATUS; ?></td>
                <td width="132" align="center" class="dataTableHeadingContent"><?php echo TABLE_HEADING_CUSTOMER_COMMENTS; ?></td>
                <td width="135" align="right" class="dataTableHeadingContent">订单来源&nbsp;</td>
                <td width="136" align="right" class="dataTableHeadingContent"><?php echo TABLE_HEADING_ACTION; ?>&nbsp;</td>
              </tr>
<form name="adv_orders" id="adv_orders" method="post" action="">
<?php
...
<?php
	...
	*/
      $orders_query_raw =   "select o.orders_id, o.customers_id, o.customers_name, o.payment_method, o.shipping_method, o.date_purchased, o.last_modified, o.currency, o.currency_value, s.orders_status_name, ot.text as order_total,o.geoip_referer" .
                            $new_fields . "
                            from (" . TABLE_ORDERS . " o, " .
                            TABLE_ORDERS_STATUS . " s " .
                            $new_table . ")
                            left join " . TABLE_ORDERS_TOTAL . " ot on (o.orders_id = ot.orders_id) " . "
                            where o.customers_id = '" . $cID . "' and o.orders_status = s.orders_status_id and s.language_id = '" . (int)$_SESSION['languages_id'] . "' and ot.class = 'ot_total' order by orders_id DESC";
...
//echo '<BR><BR>I SEE B: ' . $orders_query_raw . '<BR><BR>';
      ...
      $orders_query_raw = "select " . $search_distinct . " o.orders_id, o.customers_id, o.customers_name, o.payment_method, o.shipping_method, o.date_purchased, o.last_modified, o.currency, o.currency_value, s.orders_status_name, ot.text as order_total,o.geoip_referer" .
                          $new_fields . "
                          from (" . TABLE_ORDERS . " o, " .
                          TABLE_ORDERS_STATUS . " s " .
                          $new_table . ")
...
//    $orders_query_numrows = '';
    ...
    if ((!isset($_GET['oID']) || (isset($_GET['oID']) && ($_GET['oID'] == $orders->fields['orders_id']))) && !isset($oInfo)) {
      ...
      if (isset($oInfo) && is_object($oInfo) && ($orders->fields['orders_id'] == $oInfo->orders_id)) {
        echo '              <tr id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . zen_href_link(FILENAME_ADVANCED_ORDERS, zen_get_all_get_params(array('oID', 'action')) . 'oID=' . $oInfo->orders_id . '&action=edit', 'NONSSL') . '\'">' . "\n";
      } else {
        echo '              <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . zen_href_link(FILENAME_ADVANCED_ORDERS, zen_get_all_get_params(array('oID')) . 'oID=' . $orders->fields['orders_id'], 'NONSSL') . '\'">' . "\n";
      }
...
?>
                <td align="right" class="dataTableContent"><input name="adv_orders_id[]" type="checkbox" id="adv_orders_id" value="<?php echo $orders->fields['orders_id'];?>"></td>
                  <td align="right" class="dataTableContent"><?php echo $show_difference . $orders->fields['orders_id']; ?></td>
				...
				<?php
					$geoip_referer=geoip_referer($orders->fields['geoip_referer']);
				?>
                <td align="right" class="dataTableContent"><?php echo ($geoip_referer[0]==0)?$geoip_referer[1]:"<a class='example5' href='ajax.html'>".$geoip_referer[1]."</a>";?><!--<a class='example5' href="ajax.html" title="Homer Defined">Outside HTML (Ajax)</a>--></td>
                <td align="right" class="dataTableContent"><?php echo '<a href="' . zen_href_link(FILENAME_ADVANCED_ORDERS, zen_get_all_get_params(array('oID', 'action')) . 'oID=' . $orders->fields['orders_id'] . '&action=edit', 'NONSSL') . '">' . zen_image(DIR_WS_IMAGES . 'icon_edit.gif', ICON_EDIT) . '</a>'; ?><?php if (isset($oInfo) && is_object($oInfo) && ($orders->fields['orders_id'] == $oInfo->orders_id)) { echo zen_image(DIR_WS_IMAGES . 'icon_arrow_right.gif', ''); } else { echo '<a href="' . zen_href_link(FILENAME_ADVANCED_ORDERS, zen_get_all_get_params(array('oID')) . 'oID=' . $orders->fields['orders_id'], 'NONSSL') . '">' . zen_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
                </tr>
<?php
      $orders->MoveNext();
    }
?>
              <tr>
                <td colspan="6"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    <td class="smallText" valign="top"><input name="quanxuan" type="button" id="quanxuan" value="全选">
                  <input name="fanxuan" type="button" id="fanxuan" value="反选">&nbsp;&nbsp;<?php echo $orders_split->display_count($orders_query_numrows, MAX_DISPLAY_SEARCH_RESULTS_ORDERS, $_GET['page'], TEXT_DISPLAY_NUMBER_OF_ORDERS); ?>                  </td>
...
?>
                  <tr>
                    <td class="smallText" align="right" colspan="2">
                      <?php
                        echo '<a href="' . zen_href_link(FILENAME_ADVANCED_ORDERS, '', 'NONSSL') . '">' . zen_image_button('button_reset.gif', IMAGE_RESET) . '</a>';
                        if (isset($_GET['search']) && zen_not_null($_GET['search'])) {
                          $keywords = zen_db_input(zen_db_prepare_input($_GET['search']));
                          echo '<br/ >' . TEXT_INFO_SEARCH_DETAIL_FILTER . $keywords;
                        }
...
?>
			 ...
			 <tr class="dataTableHeadingRow">
                <td colspan="10" align="center" class=""></td>
                </tr>
			 <tr class="dataTableHeadingRow">
			   <td colspan="10" align="center" class="">
			   <input name="all_del" type="button" id="all_del" value="批量删除">
			   <input name="action" type="hidden" id="action"><?php echo zen_draw_checkbox_field('restock').TEXT_INFO_RESTOCK_PRODUCT_QUANTITY;?></td>
			   </tr>
			 <tr class="dataTableHeadingRow">
			   <td colspan="10" align="center" class=""><table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top:5px; border-top-color:#0000FF; border-top-width:medium;">
                   <tr>
                     <td width="56%" rowspan="3" align="right"><?php echo TABLE_HEADING_COMMENTS; ?><textarea style="width:500px;" name="textarea" cols="10" rows="3"></textarea></td>
                     <td width="14%"><?php echo ENTRY_STATUS; ?><?php echo zen_draw_pull_down_menu('status', $orders_statuses, $orders_status); ?></td>
                     <td width="30%" rowspan="3"><input name="all_update" type="button" id="all_update" value="批量更新状态"></td>
                   </tr>
                   <tr>
                     <td><?php echo ENTRY_NOTIFY_CUSTOMER; ?><?php echo zen_draw_checkbox_field('notify', '', true); ?></td>
                   </tr>
                   <tr>
                     <td><?php echo ENTRY_NOTIFY_COMMENTS; ?><?php echo zen_draw_checkbox_field('notify_comments', '', true); ?></td>
                   </tr>
                 </table></td>
			   </tr></form>
            </table>
			...
			$("document").ready(function(){
				$("#quanxuan").click(function(){
					$("[name='adv_orders_id[]']").attr("checked",'true');//全选
				})
				$("#fanxuan").click(function(){
					$("[name='adv_orders_id[]']").each(function(){
...
<?php
  ...
  switch ($action) {
    case 'delete':
      ...
      $contents = array('form' => zen_draw_form('orders', FILENAME_ADVANCED_ORDERS, zen_get_all_get_params(array('oID', 'action')) . 'oID=' . $oInfo->orders_id . '&action=deleteconfirm', 'post', '', true));
//      $contents[] = array('text' => TEXT_INFO_DELETE_INTRO . '<br /><br /><strong>' . $cInfo->customers_firstname . ' ' . $cInfo->customers_lastname . '</strong>');
      ...
      $contents[] = array('align' => 'center', 'text' => '<br />' . zen_image_submit('button_delete.gif', IMAGE_DELETE) . ' <a href="' . zen_href_link(FILENAME_ADVANCED_ORDERS, zen_get_all_get_params(array('oID', 'action')) . 'oID=' . $oInfo->orders_id, 'NONSSL') . '">' . zen_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
      break;
    default:
      if (isset($oInfo) && is_object($oInfo)) {
        $heading[] = array('text' => '<strong>[' . $oInfo->orders_id . ']&nbsp;&nbsp;' . zen_datetime_short($oInfo->date_purchased) . '</strong>');

        $contents[] = array('align' => 'center', 'text' => '<a href="' . zen_href_link(FILENAME_ADVANCED_ORDERS, zen_get_all_get_params(array('oID', 'action')) . 'oID=' . $oInfo->orders_id . '&action=edit', 'NONSSL') . '">' . zen_image_button('button_edit.gif', IMAGE_EDIT) . '</a> <a href="' . zen_href_link(FILENAME_ADVANCED_ORDERS, zen_get_all_get_params(array('oID', 'action')) . 'oID=' . $oInfo->orders_id . '&action=delete', 'NONSSL') . '">' . zen_image_button('button_delete.gif', IMAGE_DELETE) . '</a>');
...
// indicate if comments exist
      ...
      if (sizeof($order->products) > 0) {
        $contents[] = array('align' => 'center', 'text' => '<a href="' . zen_href_link(FILENAME_ADVANCED_ORDERS, zen_get_all_get_params(array('oID', 'action')) . 'oID=' . $oInfo->orders_id . '&action=edit', 'NONSSL') . '">' . zen_image_button('button_edit.gif', IMAGE_EDIT) . '</a>');
      }
	  if($oInfo->geoip_referer!="")
	  {
	  $geoip_referer=$db->Execute("select geoip_referer from geoip where ipid=".$oInfo->geoip_referer);
	  if ($geoip_referer->RecordCount() > 0){
		$referer=search($geoip_referer->fields['geoip_referer']);
		foreach($referer as $key=>$value)
		{
			$contents[]=array('align' => 'left', 'text' => '<font color=red>'.$key.':</font>'.$value);
		}
	  }
	  }
      break;
...
<?php
function geoip_referer($id)
{
	global $db;
	if($id!="")
	{
	$sql="select ipid,geoip_referer from geoip where ipid=".$id;
	$check_page = $db->Execute($sql);
	if ($check_page->RecordCount() > 0){
		if($check_page->fields['geoip_referer'])
		{
			$host=parse_url($check_page->fields['geoip_referer']);
			return array($check_page->fields['ipid'],$host['host']);
		}
		else
		{
			return array("0","未知");
...
{
	$value=$value."&";
	preg_match_all("/[?|&]{1}q=(.*?)&/",$value,$all);
	return array("type"=>"google_search","keyword"=>keyword_repace($all[1][0]));
}
function google_images($value)
{
	$value=$value."&";
	preg_match_all("/[?|&]{1}imgurl=(.+?)&imgrefurl=(.+?)&/",$value,$all);
	preg_match_all("/\/images%3Fq%3D(.+?)%26/",$value,$alls);
	return array("type"=>"google_images","keyword"=>keyword_repace($alls[1][0]),"imgurl"=>$all[1][0],"imgrefurl"=>$all[2][0]);
}
function google_url($value)
{
	$value=$value."&";
	preg_match_all("/[?|&]{1}url=(.+?)&rct=j&q=(.+?)&/",$value,$all);
	return array("type"=>"google_url","keyword"=>keyword_repace($all[2][0]),"url"=>keyword_repace($all[1][0]));

}
function yahoo_search($value)
{
	$value=$value."&";
	preg_match_all("/[?|&]{1}p=(.*?)&/",$value,$all);
	return array("type"=>"yahoo_search","keyword"=>keyword_repace($all[1][0]));
}
function keyword_repace($value)
{
	$search=array("+","%2B","%3A","%2F");
	$replace=array(" "," ",":","/");
	return str_replace($search,$replace,$value);
}
function search($value)
{
	if(preg_match("/google.*?\/search/",$value))
	{
		return google_search($value);
	}
	//google images
	if(preg_match("/google.*?\/imgres/",$value))
	{
		return google_images($value);
	}
	//google url
	if(preg_match("/google.*?\/url/",$value))
	{
		return google_url($value);
	}
	if(preg_match("/yahoo.com\/.*?search/",$value))
	{
		return yahoo_search($value);
	}
	else
	{
