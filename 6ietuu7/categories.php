 ...
 */
  ...
  if (zen_not_null($action)) {
    switch ($action) {
      ...
      if($_POST['categories_name'][1] != '') {
      $_POST['categories_name'][2] = ($_POST['categories_name'][2]!='')?$_POST['categories_name'][2]:$gt->english_to_italian($_POST['categories_name'][1]);
      }
      if($_POST['categories_description'][1] != '') {
      $_POST['categories_description'][2] = ($_POST['categories_description'][2]!='')?$_POST['categories_description'][2]:$gt->english_to_italian($_POST['categories_description'][1]);
      }
      ...
      //------------------------- 生成图片的地方加在这里,省事....-------------------------//
      //上传的图片被临时存放在一个叫image_tmp的文件夹里,这里的工作 就是把这些图片加上水印切割大小并存放至相应的目录
      //生成完后便删除原图
      case 'create_image':
        //这里是为描述里的图片
        if($handle_image = @opendir(DIR_FS_CATALOG_IMAGES_TMP)) {
            while(false !== ($file_image = readdir($handle_image))) {
                if($file_image != "." && $file_image != ".." && $file_image != 'main') {               
                    if(preg_match("|\.jpg$|i",$file_image)) {
                        list($width,$height) = getimagesize(DIR_FS_CATALOG_IMAGES_TMP . 'des/' . $file_image);
                        $im = imagecreatefromjpeg(DIR_FS_CATALOG_IMAGES_TMP . 'des/' . $file_image);
                        $ims = imagecreatetruecolor(400, 30);
                        $grey = imagecolorallocate($ims, 200, 200, 200);

                        $font = DIR_WS_INCLUDES . 'Fonts/STCAIYUN.TTF';

                        imagettftext($im, 20*$width/500, 45, $width*5/9, $height*8/9, $grey, $font, 'GoLouisVuitton.com');
                        imagejpeg($im, DIR_WS_PRODUCTS_IMAGES . 'org/' . $file_image);
                       // @unlink(DIR_FS_CATALOG_IMAGES_TMP . $file_image);
                    }
                }
            }
        }
        //这里是为产品图片
        if($handle_image = @opendir(DIR_FS_CATALOG_IMAGES_TMP . 'main')) {
            while(false !== ($file_image = readdir($handle_image))) {
                if($file_image != "." && $file_image != "..") {               
                    if(preg_match("|\.jpg$|i",$file_image)) {
                        
                        $imo = imagecreatefromjpeg(DIR_FS_CATALOG_IMAGES_TMP . 'pro/' . $file_image);
                        list($width_o,$height_o) = getimagesize(DIR_FS_CATALOG_IMAGES_TMP . 'pro/' . $file_image);
                        
                        $proportion_i = $width_o/$height_o;
                        if($proportion_i > $proportion) {
                            $wb = $width_o;
                            $hb = $wb/$proportion;
                        } else if($proportion_i < $proportion) {
                            $hb = $height_o;
                            $wb = $hb*$proportion;
                        } else {
                            $hb = $height_o;
                        ...
                        $x = ($wb-$width_o)/2;
                        $y = ($hb-$height_o)/2;
                        $image_b = imagecreatetruecolor($wb,$hb);
                        $white = imagecolorallocate($image_b,255,255,255);
                        imagefill($image_b,0,0,$white);
                        $imageo = imagecopyresampled($image_b,$imo,$x,$y,0,0,$width_o,$height_o,$width_o,$height_o);
                        imagejpeg($image_b, DIR_FS_CATALOG_IMAGES_TMP . 'org/' . $file_image);
                        
                        list($width,$height) = getimagesize(DIR_FS_CATALOG_IMAGES_TMP . 'org/' . $file_image);
                        $im = imagecreatefromjpeg(DIR_FS_CATALOG_IMAGES_TMP . 'org/' . $file_image);
                        
                        if($width >= $height){
                            $widl = 300;
                            $heil = $height*$widl/$width;

                            $widm = 150;
                            $heim = $height*$widm/$width;
    
                            $wids = 42;
                            $heis = $height*$wids/$width;
                        } else {
                            $heil = 300;
                            $widl = $width*$heil/$height;
    
                            $heim = 150;
                            $widm = $width*$heim/$height;
    
                            $heis = 42;
                            $wids = $width*$heis/$height;
                        }
                        //小图无水印
                        $image_s = imagecreatetruecolor($wids, $heis);
                        $image_m = imagecreatetruecolor($widm, $heim);
                        $image_l = imagecreatetruecolor($widl, $heil);
                        
                        $images = imagecopyresampled($image_s,$im,0,0,0,0,$wids,$heis,$width,$height);
                        $imagem = imagecopyresampled($image_m,$im,0,0,0,0,$widm,$heim,$width,$height);
                        $imagel = imagecopyresampled($image_l,$im,0,0,0,0,$widl,$heil,$width,$height);
                        
                        imagejpeg($image_s, DIR_WS_PRODUCTS_IMAGES . 'small/' . $file_image);
                        imagejpeg($image_m, DIR_WS_PRODUCTS_IMAGES . 'medium/' . $file_image);
                        imagejpeg($image_l, DIR_WS_PRODUCTS_IMAGES . 'large/' . $file_image);
                        //添加水印
                        $ims = imagecreatetruecolor(400, 30);
                        $grey = imagecolorallocate($ims, 200, 200, 200);
                        $font = DIR_WS_INCLUDES . 'Fonts/STCAIYUN.TTF';
...
<?php } ?>
  <tr>
<!-- body_text //-->
    <td width="100%" valign="top"><a href="<?php echo zen_href_link(FILENAME_CATEGORIES,'action=create_image');?>"><?php echo zen_image_button('button_create_image.gif', IMAGE_ORDERS_INVOICE);?></a>
<?php
