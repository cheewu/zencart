...
if (!defined('IS_ADMIN_FLAG')) {
  die('Illegal Access');
    ...
    if (zen_not_null($_POST)) {
      ...
      if($products_name[1] != '') {
      $products_name[2] = ($products_name[2]!='')?$products_name[2]:$gt->english_to_italian($products_name[1]);
      }
      $products_description = $_POST['products_description'];
      if($products_description[1] != '') {
        if($products_description[2] == '') {
            $post_products_description = $products_description[1];
            while(strlen($post_products_description)> 0) {
                $products_description[2] .= $gt->english_to_italian(substr($post_products_description,0,1500));
                $post_products_description = substr($post_products_description,1500);
            }
        }
    
      }
    //   $products_description[2] = $products_description[2]!=''?$products_description[2]:$gt->english_to_italian($products_description[1]);
...
//auto replace with defined missing image
            ...
            if ($_POST['image_delete'] == 1 || $products_image_name == '' and PRODUCTS_IMAGE_NO_IMAGE_STATUS == '1') {
              ...
              echo zen_image(DIR_FS_CATALOG_IMAGES_TMP . 'main/' . $products_image_name, $pInfo->products_name, SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, 'align="right" hspace="5" vspace="5"') . $pInfo->products_description;
            }
            echo ($products_image1_name != '')?'<div>' . zen_image(DIR_FS_CATALOG_IMAGES_TMP . $products_image1_name, $pInfo->products_name, MEDIUM_IMAGE_WIDTH, MEDIUM_IMAGE_HEIGHT, '') . '</div>':'' . '<br />';
            echo ($products_image2_name != '')?'<div>' . zen_image(DIR_FS_CATALOG_IMAGES_TMP . $products_image2_name, $pInfo->products_name, MEDIUM_IMAGE_WIDTH, MEDIUM_IMAGE_HEIGHT, '') . '</div>':'' . '<br />';
            echo ($products_image3_name != '')?'<div>' . zen_image(DIR_FS_CATALOG_IMAGES_TMP . $products_image3_name, $pInfo->products_name, MEDIUM_IMAGE_WIDTH, MEDIUM_IMAGE_HEIGHT, '') . '</div>':'' . '<br />';
            echo ($products_image4_name != '')?'<div>' . zen_image(DIR_FS_CATALOG_IMAGES_TMP . $products_image4_name, $pInfo->products_name, MEDIUM_IMAGE_WIDTH, MEDIUM_IMAGE_HEIGHT, '') . '</div>':'' . '<br />';
            echo ($products_image5_name != '')?'<div>' . zen_image(DIR_FS_CATALOG_IMAGES_TMP . $products_image5_name, $pInfo->products_name, MEDIUM_IMAGE_WIDTH, MEDIUM_IMAGE_HEIGHT, '') . '</div>':'' . '<br />';
            echo ($products_image6_name != '')?'<div>' . zen_image(DIR_FS_CATALOG_IMAGES_TMP . $products_image6_name, $pInfo->products_name, MEDIUM_IMAGE_WIDTH, MEDIUM_IMAGE_HEIGHT, '') . '</div>':'' . '<br />';
          ?>
        </td>
      </tr>
<?php
...
/* Re-Post all POST'ed variables */
      ...
      echo zen_draw_hidden_field('products_image1', stripslashes($products_image1_name));
      echo zen_draw_hidden_field('products_image2', stripslashes($products_image2_name));
      echo zen_draw_hidden_field('products_image3', stripslashes($products_image3_name));
      echo zen_draw_hidden_field('products_image4', stripslashes($products_image4_name));
      echo zen_draw_hidden_field('products_image5', stripslashes($products_image5_name));
      echo zen_draw_hidden_field('products_image6', stripslashes($products_image6_name));
